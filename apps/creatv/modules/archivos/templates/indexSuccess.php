...
while($row = mssql_fetch_array($result)){  
    ...
    $xml="        
        ...
        while($rowx = mssql_fetch_array($resultx)){
            ...
            $PLACEHOLDER='';
            if(utf8_encode($row['descripcion_tc'])=='CONTENIDO INFOCRMATIVO' || utf8_encode($row['descripcion_tc'])=='DOCUMENTALES' || utf8_encode($row['descripcion_tc'])=='ESPECIALES' || utf8_encode($row['descripcion_tc'])=='NOTICIEROS' || utf8_encode($row['descripcion_tc'])=='SERIES' || utf8_encode($row['descripcion_tc'])=='UNITARIOS') $PLACEHOLDER='PROGRAMA';
