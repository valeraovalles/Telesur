    ...
    if (Orden_compra::Verificar_proveedor(trim($refsol),trim($rifpro),&$rifpro,&$msg,&$cancotpril,&$strrifpro,&$srtrefcot))
    {
      ...
      if (Herramientas::BuscarDatos($sql,&$result))
      {
        ...
        while ($i<count($result))
        {
           if ($result[$i]['canord']<$result[$i]['canreq'])
           {
              $moneda=H::getX_vacio('REQART', 'Casolart', 'Tipmon', $result[$i]['reqart']);
              $valor=H::getX_vacio('REQART', 'Casolart', 'Valmon', $result[$i]['reqart']);
                ...
                if (Herramientas::BuscarDatos($sql1,&$result1))
                {
                 ...
                 if ($result[$i]['costo']>0) {
                  ...
                  if ($result[$i]['canreq']>0 && $result[$i]['costo']>0){
                    if ($moneda2!=$moneda) $grid['cancost'] = $result[$i]['canreq'] * ($result[$i]['costo']/$valor);
                    else $grid['cancost'] = $result[$i]['canreq'] * $result[$i]['costo'];
                  }
                  else
                      $grid['cancost'] = "0.00";
...
//<!-----------------------------------Funciones del codigo probadas----------------------------------------------------------->
    ...
    if (count($grid_detalle_orden_arreglos)>0)
    {
        $j=0;
          while ($j<count($grid_detalle_orden_arreglos))
          {
            ...
            {
              if ($tiporec=='C') {
               if ($moneda2!=$moneda) $elmonto=H::toFloat(str_replace("'","",$grid_detalle_orden_arreglos[$j][$campo13]))*H::toFloat($valor);
               else $elmonto=str_replace("'","",$grid_detalle_orden_arreglos[$j][$campo13]);
              }
              else {
               if ($moneda2!=$moneda) $elmonto=((H::toFloat($grid_detalle_orden_arreglos[$j][$campo13])*H::toFloat($valor))-$grid_detalle_orden_arreglos[$j][$campo12]);
  ...
  public static function Grabar_distribucion_recargo($caordcom,$grid_detalle_orden_arreglos,$grid_detalle_recargo,$referencia)// Aqui se usan los grid como arreglos
  {
   ...
   public static function Grabar_compromiso($caordcom)
   {
      ...
      if ($caordcom->getRefsol()=="") $moneda=$caordcom->getTipmon();
    else $moneda=Herramientas::getX('reqart','Casolart','Tipmon',$caordcom->getRefsol());
    $moneda2=H::getX_vacio('CODEMP', 'Opdefemp', 'Codmon', '001');
    if ($moneda2!=$moneda) {
 ...
 public static function chequear_disponibilidad_incremento_recargo2($caordcom,&$total_ajuste)
        ...
        {         
              ...
              if ($caordcom->getOrdcom()=='########')
              {
	            ...
	            while (!$encontrado)
	            {
	              $numero=str_pad($r, 8, '0', STR_PAD_LEFT);
                    ...
                    if (($caordcom->getTipord()=='S') || ($caordcom->getTipord()=='M') || ($caordcom->getTipord()=='T') || ($caordcom->getTipord()=='G'))
                     {
                ...
                if (($caordcom->getTipord()=='S') || ($caordcom->getTipord()=='M') || ($caordcom->getTipord()=='T') || ($caordcom->getTipord()=='G'))
		 {
          ...
          // campos
            ...
            //$grid_detalle_entregapda_objetos=$arreglo_objetos[3];
            ...
            if ($caordcom->getOrdcom()!='')
            {
              ...
              //self::Grabar_grid_entregasPDA($caordcom,$grid_detalle_entregapda_objetos);//grabo en el grid entrega PDA
      ...
      public static function Grabar_grid_resumen2($caordcom,$grid_resumen,$referencia)
  {
      ...
      if ($grid_resumen[$i]['codart']!="")
      {
        ...
        $caresordcom_new->setCosto(str_replace("'","",$grid_resumen[$i]['costo']*$valor));
        $caresordcom_new->setRgoart(str_replace("'","",$grid_resumen[$i]['rgoart']*$valor));
        $caresordcom_new->setTotart(str_replace("'","",$grid_resumen[$i]['totart']*$valor));
    ...
    if (count($grid_detalle)>0)
    {
      ...
      $a=0;
        while ($i<count($grid_detalle))
        {
          ...
          $caartord_new->setPreart(H::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col9]),3)*H::toFloat($valor));
          $result=array();
        ...
        if ($actcosart=='S')
        {
          ...
          if ($arti)
          {
              $precio=H::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col9]))*H::toFloat($valor);
          ...
          if (Herramientas::BuscarDatos($sql,&$result))
          {
            if (Herramientas::toFloat($result[0]['preart'])!=(Herramientas::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col9])))*H::toFloat($valor))
            {
                $precio=H::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col9]))*H::toFloat($valor);
          ...
          $caartord_new->setRgoart(H::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col11]))*H::toFloat($valor));
          $caartord_new->setCodrgo($codrgo);
          $caartord_new->setTotart(H::toFloat(str_replace("'","",$grid_detalle[$i][$campo_col12]))*H::toFloat($valor));
         ...
         if ($refprc=='N' && $afeprc=='S' && $afecom=='S' && $afedis=='R')
         {
		    ...
		    $cadenarec=split('!',$arreglo_grid[$j]["datosrecargo"]);
		        ...
		        while ($r<(count($cadenarec)-1))
		        {
		          ...
		          $distribucion->setMonrgo($montorecargo*H::toFloat($valor));
		          $distribucion->setTipdoc($caordcom->getDoccom());
		          $distribucion->save();
		          }
		          $r++;
