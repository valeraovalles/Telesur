...
/**
 * Base class that represents a row from the 'est_productos' table.
 *
 * 
 *
 ...
 */
abstract class BaseEstProductos extends BaseObject  implements Persistent {
	 ...
	 * @return     EstProductos The current object (for fluent API support)
	 */
	public function setIdProducto($v)
	{
		...
		if ($this->id_producto !== $v) {
			$this->id_producto = $v;
			$this->modifiedColumns[] = EstProductosPeer::ID_PRODUCTO;
		}
	...
	public function setDescripcion($v)
	{
		...
		if ($this->descripcion !== $v) {
			$this->descripcion = $v;
			$this->modifiedColumns[] = EstProductosPeer::DESCRIPCION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = EstProductosPeer::NUM_COLUMNS - EstProductosPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating EstProductos object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = EstProductosPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseEstProductos:delete:pre') as $callable)
			{
			...
			if ($ret) {
				EstProductosPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseEstProductos:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseEstProductos:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseEstProductos:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				EstProductosPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = EstProductosPeer::doInsert($this, $con);
					...
					$affectedRows += EstProductosPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = EstProductosPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = EstProductosPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = EstProductosPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EstProductosPeer::ID_PRODUCTO)) $criteria->add(EstProductosPeer::ID_PRODUCTO, $this->id_producto);
		if ($this->isColumnModified(EstProductosPeer::DESCRIPCION)) $criteria->add(EstProductosPeer::DESCRIPCION, $this->descripcion);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(EstProductosPeer::ID_PRODUCTO, $this->id_producto);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of EstProductos (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     EstProductos Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     EstProductosPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new EstProductosPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this EstProductos has previously been saved, it will retrieve
	 * related EstSolicitudess from storage. If this EstProductos is new, it will return
		...
		if ($this->collEstSolicitudess === null) {
			if ($this->isNew()) {
			   $this->collEstSolicitudess = array();
				...
				$criteria->add(EstSolicitudesPeer::ID_PRODUCTO, $this->id_producto);
	 ...
	 * Otherwise if this EstProductos is new, it will return
	 * an empty collection; or if this EstProductos has previously
	 * been saved, it will retrieve related EstSolicitudess from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in EstProductos.
	 */
	public function getEstSolicitudessJoinSfGuardUserProfile($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EstProductosPeer::DATABASE_NAME);
		}
		...
		if ($this->collEstSolicitudess === null) {
			if ($this->isNew()) {
				...
				$this->collEstSolicitudess = EstSolicitudesPeer::doSelectJoinSfGuardUserProfile($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseEstProductos:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseEstProductos::%s', $method));
	  }
